...
describe('getIconJsx', () => {
  ...
  test('returns null for non-existent icon', () => {
    const result = getIconJsx({ provider: 'lucide', key: 'non-existent-icon-name' })
    expect(result).toBeNull()
  })
  ...
  test('defaults to lucide provider when not specified', () => {
    const result = getIconJsx({ key: 'archive' })
    expect(result).toBeTruthy()
    expect(result?.type).toBe('svg')
  })
  ...
  test('svg has correct dimensions', () => {
    const result = getIconJsx({ provider: 'lucide', key: 'archive' })
    const svgString = renderToString(result!)
    expect(svgString).toMatchInlineSnapshot(`"<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><rect width="20" height="5" x="2" y="3" rx="1"/><path d="M4 8v11a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8m-10 4h4"/></g></svg>"`)
  })
})
