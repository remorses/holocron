...
function MentionUser({ url, children }: { url: string; children?: React.ReactNode }) {
  return (
...
function Audio({ source, color, children }: { source: string; color?: Color; children?: React.ReactNode }) {
  return (
...
function File({ source, color, children }: { source: string; color?: Color; children?: React.ReactNode }) {
  const displayText = children || 'Download File'
  return (
    <div className='my-4 p-4 border border-gray-200 rounded-md bg-gray-50'>
      <a href={source} download className='block no-underline text-blue-500 font-medium'>
        {displayText}
      </a>
    </div>
  )
...
function Image({ source, color, children }: { source: string; color?: Color; children?: React.ReactNode }) {
  return (
    <figure className='my-4'>
      <img src={source} alt={typeof children === 'string' ? children : ''} className='rounded-lg w-full' />
      {children && <figcaption className='mt-2 text-sm text-center text-muted-foreground'>{children}</figcaption>}
    </figure>
  )
}

function Pdf({ source, color, children }: { source: string; color?: Color; children?: React.ReactNode }) {
  return (
    <div className='my-4 p-4 border border-gray-200 rounded-md bg-gray-50'>
      <a
        href={source}
...
function Video({ source, color, children }: { source: string; color?: Color; children?: React.ReactNode }) {
  return (
    <div className='my-4'>
...
function Page({ url, color, children }: { url?: string; color?: Color; children?: React.ReactNode }) {
  if (!url) {
    return <div className='my-2 p-3 border border-border rounded-lg bg-muted'>{children}</div>
  }
  return (
    <a href={url} className='block my-2 p-3 border border-border rounded-lg hover:bg-muted no-underline'>
      {children}
    </a>
  )
}
...
function DataSource({ url, children }: { url: string; children?: React.ReactNode }) {
  return (
    <a href={url} className='block my-2 p-3 border border-border rounded-lg hover:bg-muted no-underline'>
      {children}
    </a>
...
function NotionCallout({ icon, color, children }: { icon?: string; color?: Color; children?: React.ReactNode }) {
  return (
...
function SyncedBlock({ url, children }: { url?: string; children?: React.ReactNode }) {
  return <div className='my-4 p-4 border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-950'>{children}</div>
}

function SyncedBlockReference({ url, children }: { url: string; children?: React.ReactNode }) {
  return <div className='my-4 p-4 border-l-4 border-purple-500 bg-purple-50 dark:bg-purple-950'>{children}</div>
}

function Unknown({ url, alt }: { url: string; alt: string }) {
  return (
    <div className='my-4 p-4 border border-border rounded-lg bg-muted'>
      <div className='text-sm text-muted-foreground'>Unknown block type: {alt}</div>
    </div>
  )
}
...
function Tr({ color, children }: { color?: Color; children?: React.ReactNode }) {
  return <tr>{children}</tr>
}

function Td({ color, children }: { color?: Color; children?: React.ReactNode }) {
...
function Span({
  ...
  return <span className={isUnderlined ? 'underline' : undefined}>{children}</span>
}
