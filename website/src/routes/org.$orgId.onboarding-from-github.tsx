...
export default function OnboardingFromGithub({ loaderData }: Route.ComponentProps) {
  ...
  const { data: branchesData, isLoading: loadingBranches } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await apiClient.api.getRepoBranches.post({
        orgId,
        owner,
        repo,
        installationId,
  ...
  const githubInstallUrl = new URL(href('/api/github/install'), env.PUBLIC_URL)
                ...
                {currentStep == 1 && (
                  <div className='pt-4'>
                    <Form method='post' className='flex flex-col gap-4'>
                      <input type='hidden' name='sync-repo' value='true' />
