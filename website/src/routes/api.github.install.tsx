      ...
      <Form className='flex  flex-col gap-6'>
        ...
        >
          {installations.map((org) => {
            return (
              <option key={org.accountLogin} value={org.accountLogin}>
                {org.accountLogin}
              </option>
            )
          })}
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const chosenOrg = url.searchParams.get(FormNames.chosenOrg)?.toString() || ''
  ...
  githubInstallationUrl.searchParams.set('redirect_uri', redirectUri.toString())
