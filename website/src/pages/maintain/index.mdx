...
The CLI **polls the OpenCode session** for the final reply instead of waiting on a single blocking request, so a run can now use the full **25-minute budget** — long runs used to fail at the 5-minute HTTP timeout.
