...
prompt: |
  ...
  Explain that every page with prompt frontmatter is maintained from its path and URL references. Explain that the Holocron skill adds generation prompts to new MDX pages by default. Cover routine prompts, paid model access, local use, and the command flags. Include a complete MDX example. Use the shared homepage hero from @/website/src/components/hero-section.tsx with a Maintain heading.
