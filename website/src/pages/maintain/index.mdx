...
**`@/`** always starts at the git repository root, so the path stays valid if you move the MDX file. **`@./`** and **`@../`** start at the MDX file. **`@https://`** marks a remote source. A file reference selects the page when that file changes. A folder reference selects it when any tracked file inside that folder changes, including files inside a git submodule. Use the parent-root path for submodule sources, for example `@/template/src/index.mdx`. GitHub release events select pages whose `@https://` references match the release URLs.
...
In **GitHub Actions**, Maintain creates `holocron/maintain-<timestamp>` before OpenCode starts and adds publish instructions to the session prompt, not the system prompt, so tasks never commit. When pages changed, the parent session commits them and runs `holocron maintain-open-pr` with the pull request title and body. Maintain keeps the GitHub token out of the OpenCode environment and uses it after the run to push the branch and open the pull request. The [GitHub Actions](./github-actions.mdx) page covers the no-args push range.
