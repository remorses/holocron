...
In **GitHub Actions**, Maintain creates `holocron/maintain-<timestamp>` before OpenCode starts and adds publish instructions to the session prompt, not the system prompt, so tasks never commit. The parent session commits the changed pages and runs `holocron maintain-open-pr`, or `holocron maintain-no-changes` when nothing changed. The model must end with exactly one of those two commands, even when no page changed; if it stops without either, Holocron reminds it once in the same session, then fails the run if neither is recorded. Maintain then pushes and opens the pull request. The [GitHub Actions](./github-actions.mdx) page covers the no-args push range.
