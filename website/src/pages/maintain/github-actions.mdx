...
When OpenCode finishes, Maintain **verifies the publish step**. Pages updated but left **uncommitted**, a commit on the maintain branch that was **not pushed**, or no **pull request** opened into `main` each fail the job with the specific missing step and a reminder to allow GitHub Actions to create pull requests. When the pull request exists, Maintain prints **`Opened <pull request url>`** and the job passes.
...
Schedule and manual runs open the pull request into the repository's **default branch**, read from the event payload (falling back to `main`), so the workflow works whether the default branch is `main`, `master`, or something else. A **push** run targets the pushed branch instead.
