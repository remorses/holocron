...
On a GitHub Actions run, Maintain creates the branch first. OpenCode updates and **commits** the selected MDX pages, then reports the outcome with one of two commands: `holocron maintain-open-pr` (pages changed) or `holocron maintain-no-changes`. Maintain then validates the pages, pushes the branch, and opens the pull request itself. If OpenCode runs neither command, the job fails. Local and parent-agent runs never create a branch or pull request.
