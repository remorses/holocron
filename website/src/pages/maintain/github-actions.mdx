...
| ------------------------------------------ | ---------------------------- |
| a branch (push, `workflow_dispatch`)       | that branch                  |
| `actions/checkout` with `ref: docs`        | `docs`                       |
| detached HEAD on a `release` event         | the release target branch    |
...
Maintain branches off the **checked-out commit**, so that commit must already be on `origin/<base>`. Otherwise the pull request would carry unrelated commits, and Maintain fails before it starts OpenCode. This is why `pull_request` workflows fail: their checkout is a merge commit that is on no branch.
...
Use a **dedicated provider key**. The OpenCode session can run any shell command except `git push` and `gh`, and it can read the environment. Maintain rejects the run if files outside the selected pages change.
