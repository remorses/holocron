...
The pull request targets the **branch that triggered the run**. Pass **`--base`** to target another branch.

| Trigger                            | Default base              |
| ---------------------------------- | ------------------------- |
| `push`                             | the pushed branch         |
| `schedule`, `workflow_dispatch`    | the repository default    |
| `release`                          | the repository default    |
...
Maintain branches off the **checked-out commit**, so that commit must already be on `origin/<base>`. Otherwise the pull request would carry unrelated commits, and Maintain fails before it starts OpenCode. Check out the base branch with `fetch-depth: 0`, for example `ref: docs` on `actions/checkout`.
