...
prompt: |
  ...
  Explain that in GitHub Actions the CLI creates holocron/maintain-<timestamp> before OpenCode starts. OpenCode commits the changed MDX pages and must end by running the hidden `holocron maintain-open-pr` or `holocron maintain-no-changes` command. The CLI then pushes the branch and opens the PR with Octokit. Never update main or other existing branches.
...
On a GitHub Actions run, Maintain creates the branch first. OpenCode updates and **commits** the selected MDX pages, then must end with exactly one of two commands: `holocron maintain-open-pr` (pages changed) or `holocron maintain-no-changes`, also when no page changed. Maintain then validates the pages, pushes the branch, and opens the pull request itself.

OpenCode often stops after editing. If it finishes without running either command, Maintain sends **one reminder** in the same session telling it to run exactly one, then lets it retry before the job would fail. If neither command is recorded after that, the job fails. Local and parent-agent runs never create a branch or pull request.
...
```diagram
  ...
  OpenCode updates MDX pages
        │
        ├── no MDX changes ──> maintain-no-changes, stop
        └── MDX files changed ──> OpenCode commits, runs maintain-open-pr
                        │
      ran neither ──> reminder ──> model retries ──> one command
                        │
                        v
        Maintain pushes, opens the PR into main
```

Keep **`GITHUB_TOKEN`** on the Maintain step so Maintain can push the new branch and open the pull request. Leave checkout credentials enabled. Do not set `persist-credentials: false`.
...
Use a **dedicated provider key**. The OpenCode session can run `git add`, `git commit`, and read-only git commands, and it can read the environment.
