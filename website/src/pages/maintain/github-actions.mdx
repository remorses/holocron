...
While the model works, Maintain **streams OpenCode's activity to the CI logs** instead of waiting silently: each task session and assistant message, and every finished tool call with its duration. A denied or failed tool call (for example a `git push`) prints its error inline, and OpenCode server warnings or errors (such as a provider rate limit) are shown too.
...
```text
  ...
  opencode WARN message=... error.error=AI_APICallError: Rate limit exceeded.
```
