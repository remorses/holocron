...
prompt: |
  Write the Mintlify migration guide from @/vite/src/schema.ts.
  Use @https://www.mintlify.com/docs/llms-full.txt for Mintlify field names Holocron accepts.
...
If you already have a Mintlify docs project, migrating to Holocron takes a few minutes. Holocron reads a Mintlify-compatible `docs.json` shape and supports the common MDX component vocabulary it renders today. Holocron is the [Mintlify open source alternative](./mintlify-open-source-alternative.mdx): same files, local Vite builds, MIT license.
