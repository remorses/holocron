...
prompt: |
  Write the Spiceflow integration guide from @/website/src/server.tsx.
  Use @https://github.com/remorses/spiceflow for the framework API.
title: Spiceflow
...
When your docs run on a custom domain, point `www` at the **same worker** as the apex and redirect it with a **301 in one hop**. Keeping `www` on a different host (Vercel, Pages, a second worker) or chaining `http://www` → `https://www` → apex adds extra redirects that slow down the first load and split link equity in search engines.

Add a middleware before `.use(holocronApp)` that rewrites the hostname, forces `https`, and keeps the path and query string:
...
Then register both hostnames as custom domains on the worker in `wrangler.jsonc`:
...
<Note>
Use `301`, not `307`. A permanent redirect lets browsers and crawlers cache the canonical host.
</Note>
