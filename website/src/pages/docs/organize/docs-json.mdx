...
prompt: |
  Write the docs.json guide from @/vite/src/schema.ts and @/website/docs.json.
title: docs.json
...
```json
{
  ...
  "assistant": { "enabled": true, "display": "floating" },
  "layout": {
    "mode": "compact",
    "maxWidth": 1200,
    "sidebarWidth": 230,
...
`compact` keeps the left navigation and removes the optional right aside. That also hides the default **sidebar** assistant, so set `assistant.display` to `floating` to keep Ask AI available. A page can override the default with its frontmatter `mode` field. Pages with authored asides or generated API examples keep the right rail automatically.
