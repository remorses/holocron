...
`compact` keeps the left navigation and removes the optional right aside. A page can override the default with its frontmatter `mode` field. Pages with authored asides or generated API examples keep the right rail automatically.
