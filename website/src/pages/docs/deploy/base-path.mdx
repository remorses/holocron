...
If using a [custom entry](/docs/custom-entry), Holocron's docs routes are base-aware. Your custom routes are controlled by your own Spiceflow app, so prefix them explicitly if they should also live under the same base path.
