...
Your docs are accessible at `https://my-docs-remorses-site.holocron.so/docs/`.
...
│  /           → home  │       │                                      │
│  /pricing    → page  │       │                                      │
│  /docs/*     → proxy ├──────►│  my-docs-remorses-site.holocron.so   │
│                      │       │  serves /docs/* routes               │
└──────────────────────┘       └──────────────────────────────────────┘
```
...
```js
// next.config.js
const DOCS_URL = 'https://my-docs-remorses-site.holocron.so'
...
```json
{
  "rewrites": [
    {
      "source": "/docs",
      "destination": "https://my-docs-remorses-site.holocron.so/docs"
    },
    {
      "source": "/docs/:path*",
      "destination": "https://my-docs-remorses-site.holocron.so/docs/:path*"
    }
  ]
}
```
...
```nginx
location /docs/ {
    proxy_pass https://my-docs-remorses-site.holocron.so/docs/;
...
app.use(
  ...
  createProxyMiddleware({
    target: 'https://my-docs-remorses-site.holocron.so',
    changeOrigin: true,
  }),
)
```
