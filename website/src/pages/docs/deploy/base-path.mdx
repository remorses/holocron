...
Base-path deployments get their own subdomain so they coexist with root deployments on the same project. The format is `{base}-{hash}-base-{project}-site.holocron.so`. Your docs are accessible at the printed URL under the `/docs/` path.
