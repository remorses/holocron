...
This renders your `api/authentication` guide first, then expands every endpoint not listed elsewhere into tag groups:

```
API Reference
├─ Authentication        ← api/authentication.mdx
├─ Users                 ← group from "..." (tag: users)
│   ├─ GET  /users
│   └─ POST /users
└─ Orders                ← group from "..." (tag: orders)
    └─ POST /orders
```
