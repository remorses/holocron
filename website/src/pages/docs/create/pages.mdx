...
prompt: |
  Write the pages and frontmatter guide from @/vite/src/lib/page-frontmatter.ts and @/vite/src/frontmatter-schema.json.
  Cover slugs, title, sidebarTitle, prompt, description, icon, local SVG page icons, mode, and OG fields.
title: Pages
...
| `mode`                | string    | Page layout mode: `"default"`, `"compact"`, `"center"`, or `"custom"`. Compact hides the sidebar assistant. Site-level compact in `docs.json` defaults `assistant.display` to `floating`. See [Layout](../customize/layout.mdx) |
...
Place the file at `public/icons/vercel.svg`. Use **`currentColor`** for `fill` and `stroke` so the mark matches Lucide icons in the sidebar. See [Icons](../customize/icons.mdx) for the full reference.
