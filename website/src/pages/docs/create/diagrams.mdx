...
prompt: |
  Write the diagrams guide from @/cli/src/diagram-fix.ts.
  Cover diagram fences and holocron diagrams fix.
title: Diagrams
...
- **Nested fences**: follows **CommonMark** fence rules, so a `diagram` fence nested inside a wrapping `mdx` block is fixed in place without swallowing the rest of the file
