...
| `--sidebar-link-radius` | `6px` | Corner radius of the hover / active / focus pill behind a nav row |
| `--sidebar-row-padding-x` | `8px` | Horizontal padding inside a nav row, so its pill extends past the label. Rows cancel it with a negative margin, so the text column does not move |
| `--sidebar-row-padding-y` | `4px` | Vertical padding inside a nav row. This is the pill's breathing room above and below the label |
| `--sidebar-row-gap` | `4px` | Space between sidebar rows. Lower this if you raise row padding, so the text-to-text rhythm stays tight |
| `--sidebar-indent` | `18px` | Indentation per nesting level. Defaults to the width of a row's leading icon/chevron slot so nested pages align under their group label |
