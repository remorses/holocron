...
| `--sidebar-row-padding-x` | `6px` | Horizontal padding inside a nav row, so its pill extends past the label. Rows cancel it with a negative margin, so the text column does not move |
