...
Set the same mode in **`docs.json`** to make it the default for the full site. Compact mode removes the right aside, so the default **sidebar** assistant is hidden. Also set `assistant.display` to `floating` so Ask AI stays available as a bottom pill:
