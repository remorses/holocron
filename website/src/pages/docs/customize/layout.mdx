...
Use **`<Aside wide>`** when the right rail should take leftover space, like API reference pages. The content column stays at its **720px** cap. Extra viewport width goes to the aside. This also lifts the page off the normal `layout.maxWidth` cap so the rail can grow with the viewport.
...
See [Aside](../components/aside.mdx) for `wide` and `width`.
