...
prompt: |
  Write the bleed guide from @/vite/src/styles/editorial.css.
  Cover bleed wrappers for videos, images, and code.
title: Bleed
...
Holocron defines a `--bleed` CSS variable that controls how far content extends past the prose column. It is **0px on mobile** and **up to 32px on desktop** (≥ 1080px). Bleed never fills the whole column gap, so code and images stay short of the right sidebar. The `.bleed` CSS class applies negative left and right margins, making the element wider than its parent.
...
Bleed never fills the whole **column gap**. On desktop it is capped to `columnGap - 16px`, so a tight `layout.columnGap` (for example 32px) still leaves space before the right sidebar.
