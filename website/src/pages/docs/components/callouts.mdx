...
prompt: |
  Write the Callout docs from @/vite/src/components/markdown/callout.tsx and @/vite/src/lib/remark-github-callouts.ts.
  Use @https://github.com/mintlify/components for Mintlify-compatible props.
title: Callouts
...
<Note>
Use `Note` for supporting information. It uses your brand color.
</Note>
...
## Colors follow your brand

Callout colors derive from `colors.primary` in `docs.json`, so they always match your site. `Note` uses the **primary** color, `Info` is neutral gray, and `Tip`, `Check` and `Warning` keep their meaning but shift slightly toward the primary hue. `Danger` always stays red.

To change a semantic color, override its CSS variable:
