...
export const app = new Spiceflow({ tracer: trace.getTracer('holocron') })
  ...
  .use(async ({ request }, next) => {
    ...
    if (request.method === 'OPTIONS' && request.parsedUrl.pathname.startsWith('/holocron-api/config-override')) {
  ...
  .get('/holocron-api/og', ({ request }: { request: Request }) => env.OG_WORKER.fetch(request))
  .use(schemaApp)
  .use(holocronApp)
