  ...
  static fromJSON(json) {
    ...
    for (let i2 = 0; i2 < json.length; i2++) {
      ...
      if (typeof part == 'number') {
        ...
        part.some((e, i22) => i22 && typeof e != 'string')
      ) {
        throw new RangeError('Invalid JSON representation of ChangeSet')
...
var Chunk = class {
  ...
  map(offset, changes) {
    ...
    for (let i2 = 0; i2 < this.value.length; i2++) {
      let val = this.value[i2],
        curFrom = this.from[i2] + offset,
        curTo = this.to[i2] + offset,
        newFrom,
        newTo
      if (curFrom == curTo) {
...
function findSharedChunks(a, b, textDiff) {
  ...
  for (let set of a)
    for (let i2 = 0; i2 < set.chunk.length; i2++)
      if (set.chunk[i2].maxPoint <= 0) inA.set(set.chunk[i2], set.chunkPos[i2])
  ...
  for (let set of b)
    for (let i2 = 0; i2 < set.chunk.length; i2++) {
      ...
      if (
        known != null &&
        (textDiff ? textDiff.mapPos(known) : known) == set.chunkPos[i2] &&
        !(textDiff === null || textDiff === void 0
          ? void 0
          : textDiff.touchesRange(known, known + set.chunk[i2].length))
      )
...
var HeightMapGap = class extends HeightMap {
  ...
  lineAt(value, type, oracle, top22, offset) {
    ...
    return new BlockInfo(
      line.from,
      line.length,
      Math.max(top22, Math.min(lineTop, top22 + this.height - lineHeight)),
      lineHeight,
      0,
    )
  }
        ...
        makeDecorator(conf) {
          return new MatchDecorator({
            ...
            decoration: (m, view, pos) => {
              ...
              return (
                this.decorationCache[code2] ||
                (this.decorationCache[code2] = Decoration.replace({
...
function balanceRange(
  ...
  function divide(children2, positions2, from2, to2, offset) {
    for (let i2 = from2; i2 < to2; ) {
      let groupFrom = i2,
        groupStart = positions2[i2],
        groupSize = nodeSize(balanceType, children2[i2])
      i2++
      for (; i2 < to2; i2++) {
...
function materialize(cursor) {
  ...
  let base2 = cursor.tree,
    i2 = base2.children.indexOf(buffer)
  let buf = base2.children[i2],
    b = buf.buffer,
    newStack = [i2]
  ...
  constructor(data, parser, extraExtensions = [], name2 = '') {
    ...
    this.extension = [
      ...
      EditorState.languageData.of((state, pos, side) => {
        let top22 = topNodeAt(state, pos, side),
          data2 = top22.type.prop(languageDataProp)
        if (!data2) return []
        let base2 = state.facet(data2),
          sub = top22.type.prop(sublanguageProp)
        if (sub) {
...
function cutTree(lang, tree, from, to, inside2) {
  ...
  for (let i2 = tree.children.length - 1; i2 >= 0; i2--) {
    let pos = tree.positions[i2],
      child = tree.children[i2],
      inner
    if (pos < to && child instanceof Tree) {
      ...
      return !inside2
        ...
        : new Tree(
            tree.type,
            tree.children.slice(0, i2).concat(inner),
            tree.positions.slice(0, i2 + 1),
            pos + inner.length,
          )
    }
  }
      ...
      if (verbose)
        console.log(
          base2 +
            ...
            ` (via ${(action & 65536) == 0 ? 'shift' : `reduce of ${parser.getName(action & 65535)}`} for ${parser.getName(term)} @ ${start}${localStack == stack ? '' : ', split'})`,
        )
      if (last) return true
