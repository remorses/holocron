...
function RollingText({ text, transition, stagger, reverse, font, color, textTransform, tag, padding, variable, state }) {
  ...
  const variableFontStyles = variable ? { fontVariationSettings: `'wght' ${variable.weight}, 'opsz' ${variable.optical}` } : {};
  const styles = `
    .${innerClassName} {
  ...
  return /* @__PURE__ */ _jsxs("div", { style: wrapperStyle, children: [/* @__PURE__ */ _jsx(Tag, { className: innerClassName, style: variableFontStyles, children: [...text].map((str, index) => {
    ...
    const motionSpanStyle = { display: "block", ...font, ...variableFontStyles };
    return /* @__PURE__ */ _jsx(motion.span, { variants: spanVariants, initial: "initial", animate: animateState, transition: { ...transition, delay }, style: motionSpanStyle, children: str === " " ? "\xA0" : str }, index);
