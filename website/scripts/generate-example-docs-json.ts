...
async function generateExampleDocsJson() {
  ...
  try {
    ...
    const files = await glob(
      '**/*.{md,mdx,jpg,jpeg,png,gif,bmp,webp,svg,ico,tif,tiff,avif,mp4,mov,avi,wmv,flv,webm,mkv,m4v,3gp,ogg,ogv}',
      {
        cwd: exampleDocsDir,
        absolute: true,
      },
    ...
    const docFiles: DocFile[] = files.map((filePath) => {
      ...
      const isMediaFile =
        /\.(jpg|jpeg|png|gif|bmp|webp|svg|ico|tif|tiff|avif|mp4|mov|avi|wmv|flv|webm|mkv|m4v|3gp|ogg|ogv)$/i.test(
          relativePath,
        )
