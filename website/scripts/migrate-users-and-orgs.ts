...
async function migrateUsersAndOrgs() {
  ...
  try {
    ...
    console.log(
      `  - ${oldGithubInstallationsResult.rows.filter((i) => i.status === 'suspended').length} suspended installations`,
    )
              ...
              if (orgMembers.size === 1) {
                ...
                if (designatedOwner === oldUser.id) {
                  if (!oldToNewOrgIdMap.has(orgRel.orgId)) {
                    oldToNewOrgIdMap.set(orgRel.orgId, personalOrgId)
                    console.log(
                      `  → Mapped shared org ${orgRel.orgId} to existing org ${personalOrgId} (designated owner)`,
                    )
                  }
                }
              }
          ...
          if (orgMembers.size === 1) {
            ...
            if (designatedOwner === oldUser.id) {
              if (!oldToNewOrgIdMap.has(orgRel.orgId)) {
                oldToNewOrgIdMap.set(orgRel.orgId, personalOrgId)
                console.log(
                  `  → Mapped shared org ${orgRel.orgId} to this user's personal org ${personalOrgId} (designated owner)`,
                )
              }
            }
          }
