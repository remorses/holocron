...
<Info>
  **Prerequisite**: Please install Node.js (version 19 or higher) before proceeding. <br />
  Please upgrade to ```fumabase.json``` before proceeding and delete the legacy ```mint.json``` file.
</Info>
...
**Step 2**: Navigate to the docs directory (where the `fumabase.json` file is located) and execute the following command:
