...
The `/holocron-api/chat` route is Holocron's hosted AI chat interface. It validates optional `holo_xxx` API keys, fetches the caller's `docs.zip`, creates the in-memory docs bash tool, calls AI SDK against Cloudflare Workers AI, and streams typed Spiceflow SSE chunks back to docs sites.
