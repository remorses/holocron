...
Every page in a Holocron site is an **MDX file** (`.mdx` or `.md`). Pages are discovered through the navigation config in `docs.json`, not by filesystem convention.

<Aside>

<Note>
Only pages listed in `docs.json` navigation appear in the sidebar. Creating an MDX file alone is not enough.
</Note>
...
The slug is the file path relative to the project root, without the extension:

| File path | Slug | URL |
|-----------|------|-----|
| `index.mdx` | `index` | `/` |
| `quickstart.mdx` | `quickstart` | `/quickstart` |
| `guides/auth.mdx` | `guides/auth` | `/guides/auth` |

Reference these slugs in your `docs.json`:
