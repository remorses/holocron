...
Holocron deploys your documentation site to **holocron.so** with a single command. Every push gets a live URL; every pull request gets its own preview.
...
Use them in downstream steps, for example to post a comment with the preview link:
...
Every branch and pull request gets its own isolated preview URL. The URL pattern is:

| Event | URL |
|---|---|
| Push to default branch | `https://{repo}-{owner}-site.holocron.so` |
| PR or non-default branch | `https://{branch}-{hash}-{repo}-{owner}-site-preview.holocron.so` |

Preview deployments are **fully independent**. Each branch has its own URL that persists across pushes. When a new commit is pushed to the same branch, the previous deployment on that branch is superseded and the URL updates instantly.
...
npx @holocron.so/cli keys create --name production --project <projectId>

# Deploy with the key
HOLOCRON_KEY=holo_xxx npx holocron deploy
```
...
```bash
npx holocron deploy [options]
```

| Option | Description |
|---|---|
| `--branch <name>` | Override the branch name for preview deployments |
