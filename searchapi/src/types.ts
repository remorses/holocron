...
export const FileSchema = z.object({
  ...
  weight: z
    .number()
    .optional()
    .describe('Optional weight for ranking in search results'),
})
...
const GetFileContentsResponseSchema = z.object({
  ...
  sha: z
    .string()
    .describe('SHA-1 hash of the original file content using Git blob format'),
...
export const GetDatasetSizeResponseSchema = z.object({
  ...
  breakdown: z
    .object({
      databaseSizeBytes: z
        .number()
        .describe('Estimated total size in bytes (same as totalSizeBytes)'),
      contentSizeBytes: z
        .number()
