...
`,
    ...
    expect(frontmatterSection?.cleanedSnippet).toMatchInlineSnapshot(
...
`,
    ...
    expect(frontmatterSection).toBeDefined()
    expect(frontmatterSection?.cleanedSnippet).toMatchInlineSnapshot(
      `"Complex Test Testing complex structures test complex yaml 42 This contains the word structures"`,
    )
...
`,
    ...
    const invalidYamlFrontmatter = frontmatterResults.results.find(
      (r) => r.filename === 'invalid-yaml.md' && r.sectionSlug === '',
    )
