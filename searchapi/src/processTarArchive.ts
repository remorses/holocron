...
import { z } from "zod";
import { DatasetsInterface, SUPPORTED_EXTENSIONS, FileSchema } from "./worker.js";
import type {
  DurableObjectRegion
} from "./worker.js";
...
export async function processTarArchive({
  ...
  await parseTar(gz, async (entry) => {
    ...
    if (buffer.byteLength >= 1_000_000) {
      console.log(`[import-tar] Skipping large file (${(buffer.byteLength / 1024 / 1024).toFixed(2)}MB): ${relativePath}`);
      return;
    }
  ...
  for (let i = 0; i < allEntries.length; i += BATCH_SIZE) {
    ...
    if (batch.length > 0) {
      ...
      console.log(`[import-tar] Batch ${batchNumber} completed in ${batchDuration.toFixed(2)}s (total: ${filesImported} files)`);
    }
  }
  
  const duration = (Date.now() - startTime) / 1000;
  console.log(`[import-tar] Successfully imported ${filesImported} files (${(totalSizeBytes / 1024 / 1024).toFixed(2)} MB) in ${duration.toFixed(2)} seconds`);
