...
export function ChatExample({ messages, toolName }: ChatExampleProps) {
  return (
    <div className="md:sticky md:top-[var(--fd-api-info-top)]">
      <div className="border rounded-lg p-4 bg-fd-card">
        <h4 className="font-semibold mb-3">Chat Example</h4>
        <div className="space-y-3 max-h-[500px] overflow-y-auto">
          {messages.map((message, index) => (
            <div key={index} className={`p-3 rounded-lg text-sm ${
              message.role === 'user'
                ? 'bg-fd-primary/10 border-l-2 border-l-fd-primary'
                : message.role === 'assistant'
                ? 'bg-fd-secondary border-l-2 border-l-fd-secondary'
                : 'bg-fd-muted/50 border-l-2 border-l-fd-muted'
            }`}>
              ...
              {message.role === 'assistant' && Array.isArray(message.content) && (
                <div className="mt-2 pt-2 border-t border-fd-border">
                  {message.content.filter(part => part.type === 'tool-call').map((tool, toolIndex) => (
                    <div key={toolIndex} className="text-xs bg-fd-background rounded p-2 mb-1">
                      <div className="font-mono font-medium">{(tool as any).toolName}</div>
                      {(tool as any).args && (
                        <div className="mt-1 text-fd-muted-foreground">
                          <pre>{JSON.stringify((tool as any).args, null, 2)}</pre>
                        </div>
                      )}
                    </div>
                  ))}
...
export function createSampleMCPServer() {
  return {
    server: {
      ...
      description: 'Provides tools for interacting with the local filesystem including reading, writing, and listing files and directories.',
          ...
          results: [
            {
              ...
              snippet: 'Researchers have developed new architectures that seamlessly integrate text, image, and audio processing...',
            },
            {
              title: 'Language Models Show Improved Reasoning',
              url: 'https://example.com/llm-reasoning',
              snippet: 'Recent studies demonstrate significant improvements in logical reasoning and mathematical problem solving...',
            },
          ],
          total_count: 150,
        },
