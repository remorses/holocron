...
var Language = class _Language {
  ...
  */
  ...
  static async load(input) {
    let bytes;
    if (input instanceof WebAssembly.Module || (input && input.toString && input.toString().includes("WebAssembly.Module"))) {
      // Handle WebAssembly.Module directly for Cloudflare Workers
      const mod = await C.loadWebAssemblyModule(input, { loadAsync: true });
      const symbolNames = Object.keys(mod);
      const functionName = symbolNames.find((key) => LANGUAGE_FUNCTION_REGEX.test(key) && !key.includes("external_scanner_"));
      if (!functionName) {
        throw new Error("Couldn't find language function in WASM file. Language functions should be named tree_sitter_[language_name].");
      }
  ...
  return async function(moduleArg = {}) {
    ...
    if (ENVIRONMENT_IS_NODE) {
      try {
        ...
        // Fallback for Cloudflare Workers - disable Node.js functionality
        var require = _require || (() => { throw new Error("require not available in Cloudflare Workers"); });
      }
    }
    ...
    if (ENVIRONMENT_IS_NODE) {
      try {
        var fs = _require("fs");
        var nodePath = _require("path");
        if (!import.meta.url.startsWith("data:")) {
          scriptDirectory = nodePath.dirname(_require("url").fileURLToPath(import.meta.url)) + "/";
        }
      } catch (e) {
        // Fallback for Cloudflare Workers - disable Node.js filesystem functionality
        var fs = null;
        var nodePath = null;
        scriptDirectory = "/";
      }
      readBinary = /* @__PURE__ */ __name((filename) => {
        if (!fs) {
          throw new Error("File system operations not available in Cloudflare Workers");
        }
    ...
    function findWasmBinary() {
      if (Module["locateFile"]) {
        const located = locateFile("tree-sitter.wasm");
        // Check if locateFile returned a WebAssembly.Module
        if (located instanceof WebAssembly.Module || (located && located.toString && located.toString().includes("WebAssembly.Module"))) {
          return located;
        }
        return located;
      }
      // Cloudflare Workers compatibility: handle invalid import.meta.url
      try {
        if (import.meta.url && typeof import.meta.url === 'string') {
          return new URL("tree-sitter.wasm", import.meta.url).href;
        }
      } catch (e) {
        // Fallback for invalid import.meta.url in Cloudflare Workers
      }
    ...
    async function instantiateArrayBuffer(binaryFile, imports) {
      try {
        // Handle WebAssembly.Module directly for Cloudflare Workers
        if (binaryFile instanceof WebAssembly.Module || (binaryFile && binaryFile.toString && binaryFile.toString().includes("WebAssembly.Module"))) {
          var instance2 = new WebAssembly.Instance(binaryFile, imports);
          return { module: binaryFile, instance: instance2 };
        }
