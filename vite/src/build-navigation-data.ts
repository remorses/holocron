...
export const HOLOCRON_DATA_CONTRACT_VERSION = 1

export const HOLOCRON_DATA_REQUIRED_EXPORTS = [
  'holocronDataContractVersion',
  'base',
  'getConfig',
  'getNavigationData',
  'runtimeTabEntries',
...
export type HolocronDataModule = {
  [Name in (typeof HOLOCRON_DATA_REQUIRED_EXPORTS)[number]]: unknown
}

type HolocronDataEntry<Data extends HolocronDataModule> = {
  holocronDataContractVersion?: number
  holocronData?: Partial<Data>
}

const DATA_CONTRACT_REBUILD_INSTRUCTION =
  'Rebuild the shell and regenerate tenant data with the same @holocron.so/vite version/contract.'

export function getHolocronDataContract<Data extends HolocronDataModule>(
  entry: HolocronDataEntry<Data>,
): Data {
  const version = entry.holocronDataContractVersion
  if (version !== HOLOCRON_DATA_CONTRACT_VERSION) {
    throw new Error(
      `[holocron] incompatible generated data: expected contract ${HOLOCRON_DATA_CONTRACT_VERSION}, ` +
      `received ${version === undefined ? 'missing' : JSON.stringify(version)}. ` +
      DATA_CONTRACT_REBUILD_INSTRUCTION,
    )
  }

  const data = entry.holocronData
  function hasRequiredExports(value: Partial<Data> | undefined): value is Data {
    return value !== undefined && HOLOCRON_DATA_REQUIRED_EXPORTS.every((name) => {
      return Object.hasOwn(value, name)
    })
  }
  if (!hasRequiredExports(data)) {
    const missingExports = HOLOCRON_DATA_REQUIRED_EXPORTS.filter((name) => {
      return !Object.hasOwn(data ?? {}, name)
    })
    throw new Error(
      `[holocron] generated data contract ${HOLOCRON_DATA_CONTRACT_VERSION} is missing required ` +
      `export${missingExports.length === 1 ? '' : 's'}: ${missingExports.join(', ')}. ` +
      DATA_CONTRACT_REBUILD_INSTRUCTION,
    )
  }
  return data
...
export async function generateHolocronData({
  ...
  const dataChunkSource = [
    ...
    ``,
    `export var holocronDataContractVersion = ${HOLOCRON_DATA_CONTRACT_VERSION};`,
    ``,
    ...
    `getConfig.holocronData = {`,
    ...HOLOCRON_DATA_REQUIRED_EXPORTS.map((name) => `  ${name},`),
    `};`,
    ``,
  ].join('\n')
