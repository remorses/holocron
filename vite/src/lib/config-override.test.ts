...
describe('holocronUrl', () => {
  it('defaults to holocron.so', () => {
    ...
    try {
      ...
      expect(holocronUrl('/holocron-api/config-override')).toMatchInlineSnapshot(`"https://holocron.so/holocron-api/config-override"`)
    } finally {
  ...
  it('uses HOLOCRON_URL without duplicating slashes', () => {
    ...
    try {
      ...
      expect(holocronUrl('/holocron-api/og?title=Hello')).toMatchInlineSnapshot(`"https://custom.example.com/holocron-api/og?title=Hello"`)
    } finally {
      if (previous === undefined) delete process.env.HOLOCRON_URL
      else process.env.HOLOCRON_URL = previous
    }
