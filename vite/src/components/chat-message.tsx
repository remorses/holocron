...
function ChatPartRenderer({
  ...
  if (part.type === 'text') {
    return (
      <div className='flex min-w-0 flex-col gap-(--prose-gap) overflow-x-hidden overflow-y-visible text-[13px]'>
        {part.jsx ?? part.text}
      </div>
    )
  }
...
function ChatNotice({ part }: { part: Extract<ChatPart, { type: 'notice' }> }) {
  return (
    <div>
      <div className='no-bleed flex items-start gap-2.5 rounded-[10px] bg-[color-mix(in_srgb,var(--background)_93%,var(--yellow))] p-2 text-foreground'>
