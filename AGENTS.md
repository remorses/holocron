...
3. **AI Widget (`<SidebarAssistant>` via `HolocronAIAssistantWidget`)**: Injected during AST processing (`buildSections`) with the page navigation row. If the page has **no asides**, chrome is a synthetic `<Aside full>` so the widget stays sticky from the top of the page. If the page has **exactly one authored `<Aside full>`**, chrome is prepended into that aside and the aside is moved to the top (after any leading `<FullWidth>`). Otherwise chrome is a regular first-section `<Aside>`, so later authored asides stay on their own section rows (`asideNodes`) next to their headings. If the first section already has an authored `<Aside full>` among other asides, the widget is prepended into that explicit full aside.
