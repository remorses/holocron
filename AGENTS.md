...
3. **AI Widget (`<SidebarAssistant>` via `HolocronAIAssistantWidget`)**: Injected during AST processing (`buildSections`) as a regular `<Aside>` in the first section, together with the page navigation row. It has the same sticky scope and layout behavior as any authored per-section aside. Later authored asides stay on their own section rows (`asideNodes`) next to their headings. If the first section has an authored `<Aside full>`, the widget is prepended into that explicit full aside instead.
