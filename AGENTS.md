...
`--sidebar-indent` is `calc(var(--sidebar-icon-size) + var(--sidebar-leading-gap))` so one nesting step equals the leading slot. Nested pages line up under the group label. Sidebar spacing tokens are `em` relative to `--sidebar-font-size` (13px default, 14px at `xl`). The variable lives on `:root` so user overrides on `:root` propagate correctly. Changing icon size or leading gap updates indent. The `fixtures/deep-nesting/` fixture exercises both cases: the Documentation tab has an icon on every page, the No Icons tab has none.
...
3.  **AI Widget (`<SidebarAssistant>` via `HolocronAIAssistantWidget`)**: Injected during AST processing (`buildSections`) as a synthetic `<Aside full>` so it sticks across the page. That synthetic aside does **not** collect later asides: they stay on their own section rows (`asideNodes`). The AI lives in `sharedAsideNodes` on the last section of its range. The renderer draws both layers in column 2: the AI cell is short and `sticky` at `--sticky-top`; per-section asides use `top: calc(var(--sticky-top) + var(--full-aside-height))`. The first row in an overlapping range also reserves that height so a short sticky containing block cannot force both layers to overlap. `--full-aside-height` defaults to `11rem` on `:root` so first paint already offsets; `SharedAsideCell` then measures the real AI cell height. Authored `<Aside full>` still collects later asides. If the first section already has an authored `<Aside full>`, the widget is prepended into that aside.
