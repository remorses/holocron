...
// .wrangler/tmp/bundle-Ywbxj7/strip-cf-connecting-ip-header.js
...
var cloudflare_default = {
  async fetch(req, env2) {
    ...
    const id = env2.DURABLE_FETCH.idFromName(url.pathname + url.search);
    return env2.DURABLE_FETCH.get(id).fetch(req);
  }
};
var DurableFetch = class extends DurableObject {
  constructor(state) {
    super(state, {});
    this.state = state;
    this.state.blockConcurrencyWhile(async () => {
      this.seq = await this.state.storage.get("seq") ?? 0;
      this.fetching = await this.state.storage.get("open") ?? false;
    });
  }
  static {
    __name(this, "DurableFetch");
  }
  seq = 0;
  // next chunk index
  fetching = false;
  ...
  async fetch(req) {
    ...
    if (!this.fetching) {
      ...
      this.fetching = true;
      await this.state.storage.put("open", true);
    ...
    const storedChunks = await this.state.storage.list({
      prefix: "c:"
    });
...
// .wrangler/tmp/bundle-Ywbxj7/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = cloudflare_default;
...
// .wrangler/tmp/bundle-Ywbxj7/middleware-loader.entry.ts
