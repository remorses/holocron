...
10. **Syntax highlight is server-only via refractor** — `HighlightedCodeBlock` in `lib/highlight-code.tsx` runs during RSC render. Do not import that module from `'use client'` files or refractor ships to the browser. The old `#prism` / `prism-noop` split existed because prismjs CJS globals crashed Dynamic Workers. Client `useEffect` highlight missed tokens after in-site navigation.
