...
## Cross-file links between imported .md files → first importer page (2026)

When inlined `a.md` links to `b.md` and `b.md` is imported by a different page (not a nav page), the link must point to b's importer page, not a dead `.md` path. `makeMdLinkRewriter` (sync.ts) rewrites such links to base64url markers `holocron-md-import:<absPath>` during inline-import resolution (markers survive serialization + .md-strip pass untouched). After the nav walk, `rewriteImportedMdLinksInPages` resolves each marker over the final `mdxContent` strings to: nav-page href → first-importer href (navigation order via `collectAllPages`) → relative fallback. Runs for cache hits too since it post-processes strings, keeping per-page cache keys unchanged.
