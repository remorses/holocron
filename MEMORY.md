...
OpenAPI tabs now support two modes. **Dedicated** (no `groups`/`pages`): endpoints auto-grouped by tag (unchanged). **Selective** (tab has `groups`/`pages`): the provider walks authored groups, replaces `METHOD /path` string entries with the operation's slug + emits only those endpoint pages, leaving MDX slugs untouched. `normalize-config.ts` keeps user groups instead of forcing `groups: []`; `virtual-tab-provider.ts` still assigns `tab.groups = result.groups` because the provider reads `tab.groups` and returns the merged tree. Same operation can render under two tabs with different `base` prefixes (e.g. `/api/get-users` + `/guide/get-users`) without href collision. (The slug-prefix field is `base`, renamed from `openapiBase`; it is shared with the changelog provider.)
...
10. **Prism excluded from SSR via `#prism` conditional import** — `code-block.tsx` imports `{ Prism } from '#prism'`. Browser condition → `prism.ts` (real prismjs + all grammars, ~500KB). SSR condition → `prism-noop.ts` (returns unhighlighted text). SSR renders plain code; client adds highlighting during hydration. This avoids bundling prismjs in the SSR build and sidesteps the CJS global issue that crashes Dynamic Workers. Same pattern as `#mermaid`.
11. **STABLE_GROUP must exclude `.react-server.` paths** — `react-dom/server.react-server.js` is an RSC guard stub with a top-level `throw`. Rolldown normally isolates it in a tiny never-imported chunk. The STABLE_GROUP regex forces all `node_modules` into one chunk, merging the guard in. The top-level `throw` executes during module initialization, crashing the import with `react-dom/server is not supported in React Server Components`. Fix: negative lookahead `(?!.*\.react-server\.)` in the STABLE_GROUP test regex. Reproduced locally with Node.js (not a Dynamic Worker issue).
12. **ANSI codes in CLI output break test URL parsing** — the deploy CLI wraps URLs in bold ANSI codes (`\x1b[1m...\x1b[22m`). Test regexes matching `Deployed!\s+(https://...)` fail because the escape sequence sits between the space and `https://`. Strip ANSI with `.replace(/\x1b\[[0-9;]*m/g, '')` before matching.

## Changelog virtual tab provider + `base` rename (2026)

Second VirtualTabProvider: `lib/changelog/provider.ts` generates ONE page from GitHub releases (`tab.changelog` = full repo URL). Uses `mode: center` (hides left nav) + `<Aside full><Note>` notice + one `<Update>` per release. Tab slug-prefix field renamed `openapiBase` → shared `base` (no back-compat). Shared `lib/virtual-page-mdx.ts` builds frontmatter + optional `<Aside full>` + body; both providers use it. Fetch failure = warn + page with `<Warning>` (never fails build). Test injects mock GitHub API via `HOLOCRON_CHANGELOG_API_URL` env from fixture `vite.config.ts` (async config awaits in-process http server listening before sync). 60s in-memory release cache keyed by baseUrl avoids dev re-sync API spam.

## AI chat usage = per-project dollar tracking via token×price table + credits (website, 2026)

Per-PROJECT monthly limit (not per-org). `UsageCounter` DO stays keyed per-org (`idFromName(orgId)`) but `chat_usage` rows store `project_id` + `cost_usd`; `checkLimit` filters `WHERE project_id=?` and sums USD. `lib/credits.ts` is the only knob: `USD_PER_CREDIT=0.001`, free=5000cr/$5, pro=50000cr/$50.

COST = exact token counts × a HARDCODED per-model USD table (`MODEL_USD_PER_1M_TOKENS`, computed by `computeUsdCost`). This is the ONLY viable approach — there is NO Cloudflare runtime API for real cost/neurons on `@cf/` models: (1) `env.AI.run` response has no cost/neurons; (2) AI Gateway `getLog().cost` is itself just `tokens×rate` (CF docs literally say "an estimation") AND is unusable at runtime — I measured it on preview: `getLog(eventId)` throws `AiGatewayLogNotFound` for 9+ seconds straight (it's a dashboard log written minutes later, not a billing API); (3) neurons are never returned per-request, only in the dashboard, and for LLMs neurons ARE derived from tokens anyway (e.g. glm 5500 neurons/Mtok × $0.011/1k = $0.06/Mtok). So we dropped all gateway/eventId/getLog/neuron code and just compute `tokens × published $/token`. Use `result.totalUsage` (NOT `result.usage`, which is only the last step) so multi-step tool chains sum correctly. DO SQLite self-migrates the additive `cost_usd` column via `PRAGMA table_info` probe (no `ADD COLUMN IF NOT EXISTS` in SQLite). lintcn fails with "error creating TS program" in `website` (env/tsconfig issue, unrelated) — rely on `pnpm typecheck`.

## Hosted deploys with Vite base — FIXED via auto-detected basePath (2026)

The Vite plugin writes `dist/.holocron/holocron-deploy.json` with the resolved base during deploy builds; `holocron deploy` reads it (`readBuiltBasePath`) and forwards it as deployment `basePath` when `--base-path` isn't passed, so the hosting worker strips the prefix on asset lookups. Base-path deploys still require Pro (server gates via `hasBasePath` in canDeploy) and get a `{base}-base-{project}` subdomain.

## base-path fixture dev 500s — FIXED (optimizeDeps pre-include)

Root cause: SSR optimizer discovered `motion/react` mid-request on first render → re-optimize → "There is a new version of the pre-bundle" 500 on the NEXT request (browsers auto-reload, API tests see 500). Fixed by pre-including `@holocron.so/vite > motion/react` in ssr/client optimizeDeps in vite-plugin.ts. Test harness also warms up servers in `e2e/helpers/test.ts` (retries only that transient error, requires 3 consecutive OKs).

## AI chat: lost answers came from buffered text + swallowed error chunks (2026)

`toUIMessageStream()` never throws on provider failures — it emits `{ type: 'error', errorText }` chunks masked as "An error occurred." unless you pass `onError`. The proxy ignored unknown chunk types and only flushed its text buffer on `text-end`, so a stream cut mid-text lost the whole answer with no error. Converter now lives in `vite/src/lib/chat-stream.ts` and always flushes. Model scratchpad tags (`<think>`) are handled by registering null-rendering components in `chat-render.tsx`, NOT by stripping strings: regex stripping needs code-fence carve-outs and can eat real answers. The widget also used to render only the FIRST notice of a whole conversation, so the "Temporary AI model" nag hid every error notice — notices now carry `severity` (color) and `display` (`once` vs `always`), which must stay independent: rate/credit limits are yellow but per-turn, so de-duplicating by color would hide them. A notice-only turn is a complete answer, but ONLY for terminal notices: the `display: 'once'` advisory is re-sent every turn, so counting it marks every empty turn as answered and the deduped repeat shows nothing at all. Gateway logs/bills the turn from an OUTER `finally` because a browser disconnect calls `return()` on the generator and skips everything after the stream loop.
