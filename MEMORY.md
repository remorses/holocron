...
## Hosted deploys ignore Vite base — assets 404 (2026-07)

`holocron deploy` uploads client assets keyed at root (`/assets/*`), but a site built with `base: '/docs/'` emits HTML referencing `/docs/assets/*` → every asset 404s on `*-site.holocron.so`. Subpath serving requires `--base-path` (Pro). For free previews, build with base `/` (libretto uses `DOCS_BASE_PATH=/` env in its vite config). CLI should probably error when built HTML has a base prefix but no `--base-path` was given.

## base-path fixture dev 500s — pre-existing regression on main

`pnpm --dir integration-tests test-e2e --project=base-path`: the two HTML page tests 500 in dev mode on main (raw markdown tests pass). Verified unrelated to the 0.27.x token changes by stash-testing. Needs its own debugging session.
