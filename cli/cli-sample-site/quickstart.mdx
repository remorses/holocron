...
<AccordionGroup>
  ...
  <Accordion icon="rectangle-terminal" title="Preview changes">
    ...
    <Note>
      ...
      ```fumabase upgrade```
      You should now be using the new ```fumabase.json``` configuration file. Feel free to delete the ```mint.json``` file from your repository.
    </Note>
  </Accordion>
</AccordionGroup>
